...
library LibPermit {
    ...
    string internal constant _DZAP_TRANSFER_WITNESS_TYPE_STRING = "DZapTransferWitness witness)DZapTransferWitness(address owner,address recipient)TokenPermissions(address token,uint256 amount)";
    ...
    function permit2Approve(address _permit2, address _owner, address _spender, address _token, uint160 _amount, bytes memory _data) internal {
        if (_data.length != 0) {
            ...
            try permit2Contract.permit(_owner, PermitSingle(PermitDetails(_token, _amount, expiration, nonce), _spender, sigDeadline), signature) {} catch Error(string memory reason) {
    ...
    function permit2WitnessTransferFrom(address _permit2, address _owner, address _recipient, address _token, uint256 _amount, bytes memory _data) internal {
        (uint256 nonce, uint256 deadline, bytes memory _signature) = abi.decode(_data, (uint256, uint256, bytes));
        IPermit2(_permit2).permitWitnessTransferFrom(PermitTransferFrom({ permitted: TokenPermissions(_token, _amount), nonce: nonce, deadline: deadline }), SignatureTransferDetails(_recipient, _amount), _owner, createWitnessTransferFromHash(_owner, _recipient), _DZAP_TRANSFER_WITNESS_TYPE_STRING, _signature);
    }

    function permit2BatchWitnessTransferFrom(address _permit2, address _owner, address _recipient, PermitBatchTransferFrom calldata permit, bytes calldata _signature) internal {
        ...
        IPermit2(_permit2).permitWitnessTransferFrom(permit, details, _owner, createWitnessTransferFromHash(_owner, _recipient), _DZAP_TRANSFER_WITNESS_TYPE_STRING, _signature);
    }

    /// @notice Handles permit2 batch witness transfer from
    function permit2BatchWitnessTransferFrom(address _permit2, address _owner, address _recipient, bytes32 _witness, PermitBatchTransferFrom calldata permit, bytes calldata _signature, string memory _witnessTypeString) internal {
        uint256 length = permit.permitted.length;
        uint256 i;
        SignatureTransferDetails[] memory details = new SignatureTransferDetails[](length);

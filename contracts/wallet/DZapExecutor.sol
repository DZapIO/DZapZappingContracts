...
contract DZapExecutor is IDZapExecutor, Ownable, Pausable {
    ...
    bytes32 private constant _VALIDATOR_SIGNED_DATA_TYPEHASH =
        keccak256("SignedValidatorData(bytes32 txId,address executor,address wallet,uint256 deadline,bytes32 data)");
    ...
    constructor(
        address _newOwner,
        address _dZapFactory,
        uint8 _quorum,
        address[] memory _executorsToAdd,
        address[] memory _validatorsToAdd
    ) Ownable(_newOwner) {
        _DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                _DOMAIN_TYPEHASH,
                keccak256(bytes("DZapExecutor")),
                keccak256(bytes("1")),
                block.chainid,
                address(this)
            )
    ...
    function isValidatorWhitelisted(address _validator) external view returns (bool) {
        // console.log("DZapRegistry isVerifierWhitelisted", _validator, _validators[_validator]);
    ...
    function deployWalletAndExecute(
        ...
        console.log("");
        console.log("---------------deployWalletAndExecute---------------", msg.sender);
        ...
        console.log("wallet exection started", walletAddress);
    ...
    function _verify(
        ...
        bytes32 msgHash = keccak256(
            abi.encode(_VALIDATOR_SIGNED_DATA_TYPEHASH, _txId, msg.sender, _walletAddress, _deadline, keccak256(_data))
        );
        ...
        // console.logBytes32(keccak256(_data));

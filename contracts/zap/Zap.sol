...
contract Zap is Ownable, ERC721Holder, ERC1155Holder, ReentrancyGuard, IZap {
    ...
    function _revokeErc1155Approvals(InputToken[] memory inputTokens) private {
        ...
        for (uint256 i = 0; i < inputTokensLength; ++i) {
            if (inputTokens[i].tokenType == TokenType.ERC1155 && inputTokens[i].transferType == InputTransferType.ApproveForSpender) LibAsset.revokeERC1155(inputTokens[i].tokenAddress, inputTokens[i].approveTo);
        }
    }
    ...
    function _getOutputTokensInitialBalance(uint256 _calldataLength, uint256 _outputLength, uint256 _outputCount, OutputToken[] memory _outputTokens) private view returns (uint256[] memory) {
        if (_calldataLength > 0) {
            ...
            for (uint256 i = _outputCount; i < outputEndIndex; ++i) {
                ...
                if (outputToken.tokenType == TokenType.ERC20) initialOutputBalances[index++] = LibAsset.getBalance(outputToken.tokenAddress, recipient);
                else if (outputToken.tokenType == TokenType.ERC1155) initialOutputBalances[index++] = LibAsset.getBalanceOfERC1155(outputToken.tokenAddress, recipient, outputToken.tokenId);
            }
    ...
    function _handleErc1155Input(InputToken memory _inputToken) private {
        ...
        } else if (_inputToken.transferType == InputTransferType.DirectTransferToSpender) LibAsset.transferERC1155(_inputToken.tokenAddress, msg.sender, _inputToken.approveTo, _inputToken.tokenId, _inputToken.amount);
    }
    ...
    function _handleERC721Output(OutputToken memory _outputToken, address _recipient) private {
        if (_outputToken.transferType == OutputTransferType.ReceiveAndTransfer) LibAsset.transferERC721(_outputToken.tokenAddress, _recipient, _outputToken.tokenId);
        else if (_outputToken.transferType == OutputTransferType.DirectTransferToRecipient) require(_recipient == LibAsset.getOwnerOfERC721(_outputToken.tokenAddress, _outputToken.tokenId), InvalidTokenOwner(_outputToken.tokenId));
    }

    function _handleERC1155Output(OutputToken memory _outputToken, address _recipient, uint256 _initialBalance) private {
        uint256 returnAmount = LibAsset.getBalanceOfERC1155(_outputToken.tokenAddress, address(this), _outputToken.tokenId) - _initialBalance;
        require(returnAmount >= _outputToken.minReturn, InvalidReturnAmount(returnAmount, _outputToken.minReturn));
        if (_outputToken.transferType == OutputTransferType.ReceiveAndTransfer) LibAsset.transferERC1155(_outputToken.tokenAddress, address(this), _recipient, _outputToken.tokenId, returnAmount);
    }
    ...
    function _handleZap(bytes calldata _data, address _referral) private {
        ...
        for (uint256 i; i < length; ++i) {
            ...
            uint256[] memory initialOutputBalances = _getOutputTokensInitialBalance(zapData[i].callData.length, zapData[i].outputLength, outputCount, outputTokens);

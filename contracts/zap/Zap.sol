...
contract Zap is Ownable, ERC721Holder, ERC1155Holder, ReentrancyGuard, IZap {
    ...
    bytes32 private constant _SIGNED_DATA_TYPEHASH =
        keccak256("SignedZapData(bytes32 txId,address user,address referral,uint256 nonce,bytes32 data)");
    ...
    function _verifySignature(bytes calldata _signature, bytes32 _msgHash) private view {
        console.log("====_verifySignature===");
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", _DOMAIN_SEPARATOR, _msgHash));
        console.log(ECDSA.recover(digest, _signature), verifier);
    ...
    function _getRecipient(OutputToken memory outputToken) private view returns (address) {
        console.log("====_getRecipient===");
        console.log(
            uint256(outputToken.transferType),
    ...
    function _getTotalAnReferralFeeAmount(
        ...
        console.log("====_getTotalAnReferralFeeAmount===");
        ...
        console.log(_amount, _fee, _referralFee);
        console.log(totalFeeAmount, referralFeeAmount);
    }

    function _execute(ZapData memory _zapData) private returns (bool success, bytes memory res) {
        console.log("====Executing===", _zapData.callTo);
    ...
    function _transferNativeFee(
        ...
        console.log("====_transferNativeFee===");
        if (_nativeFee != 0) {
        ...
        if (_totalNativeFeeAmount != 0) {
            ...
            console.log("_totalNativeFeeAmount", _totalNativeFeeAmount, _totalReferralNativeFeeAmount);
        }
    }
    ...
    function _transferTokenFee(
        ...
        if (_totalFeeAmount != 0) {
            ...
            console.log(_referralAddress, _totalFeeAmount, _referralFeeAmount);
        }
    }

    function _revokeErc1155Approvals(InputToken[] memory inputTokens) private {
        console.log("====_revokeErc1155Approvals===");
        uint256 inputTokensLength = inputTokens.length;
        for (uint256 i = 0; i < inputTokensLength; ++i) {
            if (
                inputTokens[i].tokenType == TokenType.ERC1155 &&
                inputTokens[i].transferType == InputTransferType.ApproveForSpender
            ) {
                console.log(i, inputTokens[i].tokenAddress);
    ...
    function _getOutputTokensInitialBalance(
        ...
        uint256 _nativeValueToTransfer,
        uint256 _outputCount,
        OutputToken[] memory _outputTokens
    ) private view returns (uint256[] memory) {
        console.log("====_getOutputTokensInitialBalance===");
        console.log(_calldataLength, _outputLength, _outputCount);
        if (_calldataLength != 0) {
            ...
            for (uint256 i = _outputCount; i < outputEndIndex; ++i) {
                ...
                console.log(i, index, uint256(outputToken.tokenType));
                address recipient = _getRecipient(outputToken);
                console.log("----recipient--", index, initialOutputBalances.length);
                ...
                if (outputToken.tokenType == TokenType.ERC20) {
                    ...
                    console.log("initialOutputBalances", initialOutputBalances[index - 1]);
                    ...
                    initialOutputBalances[index++] = LibAsset.getBalanceOfERC1155(
    ...
    function _handleNativeInput(
        ...
        console.log("====_handleNativeInput===", uint256(_inputToken.transferType));
        if (_tokenFee != 0)
    ...
    function _handleErc20Input(
        ...
        console.log("====_handleErc20Input===", uint256(_inputToken.transferType));
    ...
    function _handleErc721Input(InputToken memory _inputToken) private {
        console.log("====_handleErc721Input===", uint256(_inputToken.transferType));

        if (_inputToken.transferType == InputTransferType.ApproveForSpender) {
            console.log("getOwnerOfERC721", LibAsset.getOwnerOfERC721(_inputToken.tokenAddress, _inputToken.tokenId));
    ...
    function _handleErc1155Input(InputToken memory _inputToken) private {
        console.log("====_handleErc1155Input===", uint256(_inputToken.transferType));
        ...
        console.log("balance", balance, _inputToken.amount);
         // if there is requirement of extra tokens
        if (balance < _inputToken.amount) 
            LibAsset.transferERC1155(_inputToken.tokenAddress, msg.sender, address(this), _inputToken.tokenId, _inputToken.amount - balance);

        if (_inputToken.transferType == InputTransferType.ApproveForSpender) {
            // remember to make if false after call
            LibAsset.approveERC1155(_inputToken.tokenAddress, _inputToken.approveTo);
        } else if (_inputToken.transferType == InputTransferType.TransferToSpender)
            LibAsset.transferERC1155(_inputToken.tokenAddress, address(this), _inputToken.approveTo, _inputToken.tokenId, _inputToken.amount);
    }
    ...
    function _handleERC20Output(
        ...
        console.log("====_handleERC20Output===", uint256(_outputToken.transferType));

        uint256 returnAmount = LibAsset.getBalance(_outputToken.tokenAddress, _recipient) - _initialBalance;
        console.log("returnAmount", returnAmount, _outputToken.minReturn);
        ...
        console.log("totalFeeAmount", totalFeeAmount, referralFeeAmount);
    ...
    function _handleERC721Output(OutputToken memory _outputToken, address _recipient) private {
        console.log("====_handleERC721Output===", uint256(_outputToken.transferType));
        ...
        if (_outputToken.transferType == OutputTransferType.ReceiveAndTransfer)
            ...
            console.log(
                "getOwnerOfERC721",
                LibAsset.getOwnerOfERC721(_outputToken.tokenAddress, _outputToken.tokenId),
                _recipient
            );
    }
    ...
    function _handleERC1155Output(
        ...
        console.log("====_handleERC1155Output===", uint256(_outputToken.transferType));
    ...
    function _processInputTokens(
        ...
        console.log("====_processInputTokens===", _inputCount, _zapData.inputLength);
        ...
        for (uint256 i = _inputCount; i < length; ++i) {
            ...
            console.log(i, uint256(inputToken.transferType));
        ...
        console.log("_processInputTokens, totalNativeFeeAmount", totalNativeFeeAmount, totalReferralNativeFeeAmount);
    }
    ...
    function _processOutputTokens(
        ...
        console.log("====_processOutputTokens===", _outputCount, _zapData.outputLength);
        ...
        for (uint256 i = _outputCount; i < length; ++i) {
            ...
            if (outputToken.tokenType == TokenType.ERC20)
                ...
                _handleERC1155Output(outputToken, recipient, _initialOutputBalances[index++]);
        }
    }
    ...
    function _handleErcDeposits(InputErc20Tokens[] calldata _inputTokens) private {
        console.log("====_handleErcDeposits===", _inputTokens.length);

        uint256 length = _inputTokens.length;

        for (uint256 i; i < length; ++i) {
            console.log(i, _inputTokens[i].token);
    ...
    function _handleSweepTokens(address[] calldata _sweepErc20, address _dustReciever) internal {
        console.log("====_handleSweepTokens===", _sweepErc20.length);

        uint256 length = _sweepErc20.length;
        for (uint256 i = 0; i < length; ++i) {
            console.log(i, _sweepErc20[i]);
    ...
    function _handleZap(bytes calldata _data, address _referral) private {
        ...
        (
            ...
            uint256 nativeFee
        ) = abi.decode(_data, (InputToken[], OutputToken[], ZapData[], uint256));
        console.log("decoded");
        ...
        for (uint256 i; i < length; ++i) {
            ...
            totalNativeFeeAmount += nativeFeeAmount;
            totalReferralNativeFeeAmount += referralNativeFeeAmount;
            console.log("processed output", outputCount, totalNativeFeeAmount, totalReferralNativeFeeAmount);
            console.log("");
        }
        console.log("");
